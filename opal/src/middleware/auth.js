...
function authenticateJWT(req, res, next) {
  ...
  logger.info(`[AUTH] Request path: ${req.path}, Authorization header: ${authHeader ? 'present' : 'missing'}`);
  
  if (!authHeader) {
    logger.warn(`[AUTH] Authorization header missing for ${req.path}`);
  ...
  if (parts.length !== 2 || parts[0] !== 'Bearer') {
    logger.warn(`[AUTH] Invalid authorization format: ${authHeader}`);
    return res.status(401).json({ error: 'Invalid authorization format' });
  }
  
  const token = parts[1];
  logger.info(`[AUTH] Token extracted: ${token.substring(0, 10)}...`);
  
  // Verify token
  const decoded = verifyToken(token);
  logger.info(`[AUTH] Token decoded: ${JSON.stringify(decoded)}`);
  
  if (!decoded) {
    logger.warn(`[AUTH] Invalid or expired token for ${req.path}`);
  ...
  logger.info(`[AUTH] User attached to request: ${JSON.stringify(req.user)}`);
