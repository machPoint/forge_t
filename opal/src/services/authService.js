 ...
 * @returns {Promise<Object>} - User data and tokens in camelCase
 */
async function login(usernameOrEmail, password) {
  try {
    logger.info(`[authService.login] Attempting login for: ${usernameOrEmail}`);
    
    // Find user by username or email
    const user = await db('users')
      .select(['id', 'username', 'email', 'password_hash', 'role']) // Explicitly select all needed fields including id
    ...
    if (!user.id) {
      logger.error(`[authService.login] User found but has no ID: ${JSON.stringify(user, null, 2)}`);
    ...
    // Generate tokens
    logger.debug(`[authService.login] Generating tokens for user: ${user.username}, ID: ${user.id}`);
    ...
    logger.info(`User logged in: ${camelCaseUser.username}`);
    return {
      user: camelCaseUser,
      accessToken,
      refreshToken
 ...
 * @returns {Promise<Object>} - Created user with camelCase keys
 */
async function register(username, email, password, role = 'user') {
  try {
    // Check if username or email already exists
    const existingUser = await db('users')
      .where({ username: toSnakeCase(username) })
      .orWhere({ email: toSnakeCase(email) })
      .first();
    ...
    logger.info(`[authService.register] Registering new user with ID: ${userId}`);
    
    // Create user with explicit ID
    const [user] = await db('users').insert({
      id: userId,  // Explicitly set the UUID
      username: toSnakeCase(username),
      email: toSnakeCase(email),
      password_hash: passwordHash,
      role
    }).returning(['id', 'username', 'email', 'role', 'created_at']);
    
    // Double-check that the user has an ID
    if (!user || !user.id) {
      logger.error(`[authService.register] User created but has no ID: ${JSON.stringify(user)}`);
...
async function refreshToken(refreshToken) {
  try {
    ...
    // Get user
    const user = await db('users')
      .where({ id: decoded.id || decoded.userId }) // Support both id and userId for backward compatibility
    ...
    // Convert user data to camelCase
    const camelCaseUser = toCamelCase(user);
    
    // Generate new access token
    const accessToken = generateAccessToken(camelCaseUser);
    
    logger.info(`Token refreshed for user: ${camelCaseUser.username}`);
    return { accessToken };
  } catch (error) {
...
function generateAccessToken(user) {
  // Ensure we have a valid user ID before generating the token
  if (!user || !user.id) {
    logger.error(`[authService.generateAccessToken] Attempted to generate token for user with missing ID: ${JSON.stringify(user)}`);
    throw new Error('Cannot generate token: User ID is missing or null');
  }
  
  logger.info(`[authService.generateAccessToken] Generating token for user ID: ${user.id}, username: ${user.username}`);
 ...
 * @param {Object} permissions - Token permissions in camelCase
 * @param {Date|null} expiresAt - Expiration date (null for no expiration)
 * @returns {Promise<Object>} - Created API token with camelCase keys
 */
async function createApiToken(userId, name, permissions = {}, expiresAt = null) {
  ...
  try {
    ...
    logger.debug(`Creating API token: ${name} for user: ${user_id} with id: ${id}`);
    ...
    // Convert snake_case keys to camelCase for frontend
    const camelCaseToken = toCamelCase(apiToken);
 ...
 * @returns {Promise<Object|null>} - Token data with camelCase keys or null if invalid
 */
async function validateApiToken(token) {
  try {
    ...
    logger.debug(`API token validated successfully for user: ${user.username}`);
 ...
 * @param {string} userId - User ID
 * @returns {Promise<Array>} - User's API tokens with camelCase keys
 */
async function getUserApiTokens(userId) {
  try {
    ...
    // Convert snake_case keys to camelCase for frontend
    const camelCaseTokens = tokens.map(token => toCamelCase(token));
...
async function deleteApiToken(tokenId, userId) {
  ...
  try {
    ...
    logger.debug(`Attempting to delete API token: ${tokenId} by user: ${userId}`);
    ...
    // Convert token to camelCase for consistent processing
    const camelCaseToken = toCamelCase(token);
    
    // Check if user has permission to delete this token
    if (camelCaseToken.userId !== userIdStr && await isAdmin(userIdStr) === false) {
