...
const MemoriesSidebar: React.FC<MemoriesSidebarProps> = ({ 
  memories = [], 
  selectedMemoryId, 
  onMemorySelect,
  showArchived = false,
  ...
  const [selectedMemory, setSelectedMemory] = useState<string | null>(selectedMemoryId || null);

  useEffect(() => {
    setSelectedMemory(selectedMemoryId || null);
  }, [selectedMemoryId]);
  ...
  return (
    <div className="h-full flex flex-col bg-[#1d1d1d] dark:bg-[#1d1d1d]">
      <div className="p-4 border-b border-gray-200 dark:border-gray-700">
        ...
        <div className="relative">
          ...
          <Input
            ...
            className="pl-10 bg-[#2a2a2a] border-gray-600 text-gray-200"
          />
        </div>
      </div>
        ...
        <div className="space-y-3">
          {isLoading ? (
            ...
            filteredMemories.map((memory) => (
              <Card
                key={memory.id}
                className={`cursor-pointer transition-all hover:bg-[#2a2a2a] ${
                  selectedMemory === memory.id ? 'ring-2 ring-gray-500' : ''
